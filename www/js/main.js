...
.controller('dialogsController', function($scope){
        ...
        return {
            onAlert:function(){
                navigator.notification.alert(
                    'This is alert',
                    function(times){ $scope.d.alert = times; },
                    'Alert',
                    ['OK', 'Cancel']
                );
            },
            onConfirm:function(){
                navigator.notification.confirm(
                    ...
                    function(times){ $scope.d.confirm = times; },
                    'Confirm',
                    ['Confirm', 'Cancel']
                );
            },
            onPrompt:function(){
                navigator.notification.prompt(
                    ...
                    function(text){ $scope.d.prompt = text; },
                    'Text',
                    ['OK', 'Cance,'],
                    'Like your name'
                );
    ...
    $scope.onAlert = function(){    $scope.onDeviceReady().onAlert();    }
    $scope.onConfirm = function(){    $scope.onDeviceReady().onConfirm();    }
    $scope.onPrompt = function(){    $scope.onDeviceReady().onPrompt();    }
    $scope.onBeep = function(){    $scope.onDeviceReady().onBeep();    }
})
.controller('motionController', function($scope){
    ...
    $scope.onDeviceReady = function(){
        ...
        navigator.accelerometer.watchAcceleration(function(acceleration){
            var yAxis = acceleration.y / 9.81 * 140;
            var xAxis = acceleration.x / 9.81 * 140;
            ball.style.top = yAxis + 'px';
            ball.style.left = xAxis + 'px';
        }, function(){
            $scope.onLoadError = "Initialization Failed";
        }, {
            frequency: 10
        })
    }
